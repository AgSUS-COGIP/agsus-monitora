...
| 1     | **Feita em 25/09** (menos `pwa-lifecycle.css`, que depende da decisão sobre `bench/`). Lixo certo: `get_analises_dashboard_payload` (v1) e suas 4 views e cache, gatilho de invalidação, `VW_AUDITORIA_ACESSOS_DIARIA`, `TL_NOTIFICACAO`, funções sem referência; `pwa-lifecycle.css`; dependências `echarts`, `pdfjs-dist`, `tesseract.js` |
| 2     | Confirmar suspeitos (`TA_ANALISE_QUARENTENA`, `bench/`, funções sem chamador)                                                                                                                                                                                                                                                               |

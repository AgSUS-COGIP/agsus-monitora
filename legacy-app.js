...
function renderAccessPanelChoices(selectedIds=[]){
  ...
  function buildExternalPanel(holder,panel){
    ...
    // Google Apps Script bloqueia carregamento dentro de iframe em domínios externos.
    // Em vez de mostrar a tela quebrada "script.google.com recusou a conexão",
    // o sistema exibe uma orientação e abre o painel em nova aba.
    if(isGoogleAppsScriptUrl(safePanelUrl)){
      holder.innerHTML=`
        <div class="external-placeholder" style="background:#f8fafc;color:#334155;padding:28px;">
          <div style="max-width:720px;margin:auto;text-align:center;">
            <div style="font-size:58px;color:#0f766e;margin-bottom:12px;"><i class="fa-solid fa-up-right-from-square"></i></div>
            <h2 style="margin:0 0 10px;font-size:24px;color:#0f172a;">${esc(panel.titulo)}</h2>
            <p style="margin:0 auto 18px;line-height:1.55;font-weight:700;color:#475569;">
              Este painel é um Google Apps Script. Por segurança, o Google não permite que ele seja aberto embutido dentro do AgSUS Monitora.
            </p>
            <p style="margin:0 auto 22px;line-height:1.55;color:#64748b;font-weight:600;">
              Use o botão abaixo para abrir o painel diretamente em uma nova aba. O botão "Voltar ao sistema" continua disponível nesta tela.
            </p>

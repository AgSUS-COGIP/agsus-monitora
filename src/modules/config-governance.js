...
const FIELD_MAP = [
  ...
  ["cfgGoogleEnabled", "auth_google_enabled", "Exibe ou oculta o login com Google", "true"],
  ["cfgGoogleButtonText", "auth_google_button_text", "Texto do botão de autenticação Google"],
  ["cfgGoogleDomainHint", "auth_google_domain_hint", "Domínio sugerido no login Google"],
  ...
  ["cfgMascot", "mascot_url", "Logo / mascote da sidebar"],
  ...
  ["cfgRealtimeEnabled", "feature_realtime_monitoramento", "Habilita atualização em tempo real do monitoramento", "true"],
  ["cfgAccessHeartbeatMinutos", "access_heartbeat_minutos", "Intervalo de auditoria heartbeat, em minutos", "5"],
  ["cfgExecutiveModeEnabled", "feature_modo_executivo", "Habilita o modo executivo no dashboard principal", "true"]
];
...
function friendlyError(error) {
  return txt(error?.message || error?.details || error?.hint || error || "Erro desconhecido");
}
...
function isDirty() {
  ...
  return Boolean(indicator && !indicator.hidden) || Boolean(saveButton && !saveButton.disabled);
}
...
function currentPanels() {
  try {
    const panelItems = [...document.querySelectorAll("#panelAdmin .panel-admin-item")];
    const placeholders = panelItems.map((_, index) => ({ id: txt($( `panelId${index}` )?.value) }));
...
function validateCurrentConfiguration() {
  ...
  if (domain && (!/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/i.test(domain) || /[:/\s@]/.test(domain))) {
  ...
  const urls = [$("cfgMascot"), $("cfgCogipLogo"), ...document.querySelectorAll('[id^="panelUrl"]')].filter(Boolean);
  urls.forEach(field => {
    ...
    try {
      ...
      errors.push(`URL inválida no campo ${field.closest(".form-row")?.querySelector("label")?.textContent || field.id}.`);
    }
  });
  ...
  document.querySelectorAll('[id^="panelAtivo"]').forEach(active => {
    ...
    if (!txt($( `panelUrl${index}` )?.value)) errors.push("Painel ativo sem URL configurada.");
  });
...
function snapshotMaps(snapshot) {
  const configMap = new Map((snapshot?.configuracoes || []).map(item => [item.chave, item]));
  const panelMap = new Map((snapshot?.paineis || []).map(item => [String(item.id), item]));
...
function ensureGovernanceUI() {
  ...
  if (!root) return;
  if (!$("configGovernanceModal")) document.body.insertAdjacentHTML("beforeend", modalHTML());
  ...
  document.querySelectorAll("[data-governance-close]").forEach(button => button.addEventListener("click", closeModal));
...
async function reviewAndPublish() {
  ...
  if (!client) return window.alert("Supabase indisponível para publicar configurações.");
  ...
  try {
    ...
    $("configConfirmPublish")?.addEventListener("click", async () => {
      ...
      if (button) {
        ...
        button.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Publicando...';
      }
      ...
      try {
        ...
        if (!data?.ok) throw new Error("O Supabase não confirmou a publicação.");

        markClean();
        closeModal();
        await loadHistory(true);
        window.dispatchEvent(new CustomEvent("agsus:config-saved", { detail: data }));
        window.alert(`${data.total_alteracoes || changes.length} alteração(ões) publicada(s) e auditada(s). A página será recarregada para aplicar os novos valores.`);
        window.location.reload();
      } catch (error) {
        if (button) {
          button.disabled = false;
          button.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Tentar novamente';
        }
...
function historyChangeSummary(changes) {
  ...
  const labels = items.slice(0, 3).map(item => item.rotulo || item.chave || item.codigo || "Alteração");
...
function historyHTML(history) {
  ...
  return `<div class="config-history-list">${history.map(item => {
    const date = item.created_at ? new Date(item.created_at) : null;
    const dateLabel = date && !Number.isNaN(date.getTime()) ? date.toLocaleString("pt-BR") : "Data não informada";
...
function bindRestoreButtons() {
  document.querySelectorAll(".config-history-restore").forEach(button => {
    button.addEventListener("click", () => reviewRestore(button.dataset.versionId));
  });
}

function reviewRestore(versionId) {
  const version = state.history.find(item => String(item.id) === String(versionId));
  if (!version) return;
  ...
  $("configConfirmRestore")?.addEventListener("click", async () => {
    ...
    const confirmed = window.confirm("Confirmar restauração desta versão? Os valores atuais serão substituídos.");
    if (!confirmed) return;

    const button = $("configConfirmRestore");
    if (button) {
      button.disabled = true;
      button.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Restaurando...';
    }
    const client = createClient();
    ...
    if (error || !data?.ok) {
      if (button) {
        button.disabled = false;
        button.innerHTML = '<i class="fa-solid fa-rotate-left"></i> Tentar novamente';
      }
    ...
    markClean();
    closeModal();
    window.alert(`${data.total_alteracoes || 0} alteração(ões) restaurada(s). A página será recarregada.`);
    window.location.reload();
  });
}

function installSaveOverride() {
  if (typeof window.saveAdminSettings !== "function" || window.saveAdminSettings.__governanceWrapped) return;
  const fallback = window.saveAdminSettings;
  const governed = async (...args) => {
    try {
      return await reviewAndPublish(...args);
    } catch (error) {
      console.error("Falha no fluxo de governança; salvamento antigo preservado como contingência.", error);
...
function installNavigationGuard() {
  if (typeof window.navigate !== "function" || window.navigate.__configGovernanceWrapped) return;
  const originalNavigate = window.navigate;
  const guardedNavigate = (...args) => {
    const target = String(args[0] || "");
    if (isDirty() && target !== "config") {
      const confirmed = window.confirm("Existem alterações não salvas em Configurações. Sair e descartar essas alterações?");

...
function numberFromCell(cell){
  const value = String(cell?.childNodes?.[0]?.textContent || cell?.textContent || "")
    .replace(/[^0-9,-]/g, "")
    .replace(/\./g, "")
    .replace(",", ".");
...
function columnIndex(label){
  const headers = [...document.querySelectorAll("#page-dashboard .details-table thead th")];
  return headers.findIndex(th => normalize(th.textContent).toLowerCase() === normalize(label).toLowerCase());
}
...
function enhanceVacancyRates(){
  ...
  document.querySelectorAll("#monitorRows tr").forEach(row => {
    ...
    if(!vagasCell || !ociosasCell || ociosasCell.querySelector(".health-vacancy-rate")) return;
    ...
    const level = rate >= 60 ? "critical" : rate >= 40 ? "high" : rate >= 20 ? "medium" : "low";
    ociosasCell.insertAdjacentHTML("beforeend", `<span class="health-vacancy-rate ${level}" title="${rate}% das vagas estão ociosas">${rate}%</span>`);
  });
}
...
function ensureSelectedBanner(){
  ...
  if(!banner){
    ...
    banner.querySelector("button")?.addEventListener("click", () => {
      const brasilButton = [...document.querySelectorAll("#map button")].find(button => normalize(button.textContent).includes("Brasil"));
...
function syncMapLegend(){
  ...
  if(legend.textContent.includes("estado atendido") && !legend.textContent.includes("selecionado")){
    legend.innerHTML = legend.innerHTML.replace("estado atendido", "estado atendido (selecionado)");
  }
}

function handleMapClick(event){
  const target = event.target;
  if(target?.closest?.("button") && normalize(target.closest("button").textContent).includes("Brasil")){
    clearSelectedDsei();
    return;
  }
  if(!target?.closest?.("#map")) return;
  window.setTimeout(() => {
    const searchValue = normalize(document.getElementById("tableSearch")?.value);
    if(searchValue && !searchValue.toLowerCase().includes("casai")) showSelectedDsei(searchValue);
    syncMapLegend();
  }, 160);
}
...
function installObservers(){
  ...
  if(rows){
    new MutationObserver(() => enhanceVacancyRates()).observe(rows, { childList:true });
    enhanceVacancyRates();
  }

  const map = document.getElementById("map");
  if(map){
    new MutationObserver(() => syncMapLegend()).observe(map, { childList:true, subtree:true });
    syncMapLegend();
  }
}

export function initHealthDashboardRefinements(){

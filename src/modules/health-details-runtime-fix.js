...
function decorateFilterFields(documentRef) {
  ...
  [...body.children].forEach((child) => {
    if (child.tagName === "BUTTON") {
      ...
      child.innerHTML =
        '<i class="fa-solid fa-eraser" aria-hidden="true"></i> Limpar todos';
      return;
    }
...
function filterMenuOptions(menu, query) {
  ...
  const options = [...menu.querySelectorAll(".multi-option")].filter(
    (option) => !option.classList.contains("empty"),
  );
  let visible = 0;
  options.forEach((option) => {
    const show =
      !normalizedQuery ||
      normalize(option.textContent).includes(normalizedQuery);
    option.hidden = !show;
    if (show) visible += 1;
  });
...
function ensureFilterMenuSearches(documentRef) {
  documentRef
    .querySelectorAll("#filterBody .multi-select")
    .forEach((select) => {
      ...
      const selectAll = actions.querySelector(
      ...
      const fieldLabel =
        select.closest(".health-filter-field")?.querySelector("label span")
          ?.textContent || "opções";
      ...
      wrap
        ...
        ?.addEventListener("input", (event) =>
          filterMenuOptions(menu, event.target.value),
        );
    });
}

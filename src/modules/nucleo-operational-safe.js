...
function summaryForRow(tr) {
  ...
  return state.summary.find(item => keyOf(item.unidade, item.edital) === keyOf(cells[0]?.textContent, cells[1]?.textContent));
}
...
export function initNucleoOperationalSafe() {
  ...
  document.addEventListener("click", event => {
    if (event.target?.closest?.('[data-view="nucleo"], [onclick*="nucleo"], #nucleoOperationalRefresh')) scheduleDecoration();
  });
}
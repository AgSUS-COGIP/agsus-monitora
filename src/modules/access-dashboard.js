...
export function createAccessDashboard(deps){
  ...
  function accessScreen(user){
    return txt(user.tela_atual) || txt(user.ultimo_evento) || "Tela não registrada";
  }
  ...
  function userStatus(user, onlineMinutes){
    ...
    if(minutes !== null && minutes <= onlineMinutes) return { label:"Online", className:"online" };
    if(minutes !== null && minutes <= 60) return { label:"Recente", className:"recent" };
    return { label:"Offline", className:"offline" };
  }

  function accessMonitorKpi(label, value, variant=""){
    const cls = ["access-monitor-kpi", variant ? `is-${variant}` : ""].filter(Boolean).join(" ");
  ...
  function render(payload){
    ...
    const onlineMinutes = n(payload.online_minutes || DEFAULT_ONLINE_MINUTES) || DEFAULT_ONLINE_MINUTES;

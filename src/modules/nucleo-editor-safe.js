...
function applyBulkDates() {
  const rawLines = String($("cronogramaBulkInput")?.value || "").split(/\r?\n/).map(line => line.trim()).filter(Boolean);
  const rows = [...document.querySelectorAll("#cronogramaRows tr[data-cronograma-index]")];
  if (!rows.length) return feedback("Use primeiro o modelo padrão para criar as atividades.", "error");
  if (!rawLines.length) return feedback("Cole pelo menos uma data ou intervalo.", "error");
  ...
  if (invalid.length) return feedback(`Não foi possível interpretar ${invalid.length === 1 ? "a linha" : "as linhas"} ${invalid.join(", ")}.`, "error");
  ...
  if (parsed.length < rows.length) feedback(`${total} datas aplicadas. Ainda faltam ${rows.length - parsed.length} etapas para preencher.`, "warning");
  else if (parsed.length > rows.length) feedback(`${total} datas aplicadas. ${parsed.length - rows.length} linhas excedentes foram ignoradas.`, "warning");
  else feedback(`${total} etapas preenchidas com sucesso. Revise antes de salvar.`, "success");
...
function enhanceEditor() {
  ...
  editor.querySelector(".cronograma-source-grid")?.classList.add("cronograma-manual-source-grid");
  editor.querySelector(".cronograma-actions-box")?.classList.add("cronograma-manual-actions");

  const heading = editor.querySelector(".cronograma-heading p");
  if (heading) heading.textContent = "Cadastre as etapas manualmente ou use o modelo padrão. Status e etapa serão calculados pelas datas salvas.";

  const exampleButton = $("cronogramaExample");
  if (exampleButton) {
    exampleButton.title = "Criar as atividades padrão e preencher as datas em lote";
    exampleButton.innerHTML = '<i class="fa-solid fa-list-check" aria-hidden="true"></i> Usar modelo padrão <span class="cronograma-recommended-badge">Recomendado</span>';
    exampleButton.addEventListener("click", () => window.setTimeout(() => {
  ...
  if (tableWrap && !$("cronogramaBulkDates")) tableWrap.insertAdjacentHTML("beforebegin", bulkMarkup());
  if (tableWrap && !editor.querySelector(".cronograma-view-hint")) {
    tableWrap.insertAdjacentHTML("beforebegin", `<div class="cronograma-view-hint"><i class="fa-solid fa-calendar-days" aria-hidden="true"></i><span>Este é o cronograma oficial do edital. Para consultá-lo novamente, abra o edital pelo botão <strong>Editar</strong>.</span></div>`);
  }

  $("cronogramaBulkClose")?.addEventListener("click", () => { $("cronogramaBulkDates").hidden = true; });

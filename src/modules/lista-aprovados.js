...
export function createListaAprovadosController(options = {}) {
  ...
  const state = {
    ...
    /*
      Página da tabela, começando em 1. A lista inteira continua em memória — o
      que a paginação corta é o custo de DESENHAR: antes cada filtragem montava
      uma string HTML com todas as linhas e a atribuía de uma vez ao `innerHTML`.
      Com milhares de candidatos isso trava o navegador, e acontecia a cada tecla
      digitada na busca, que escuta o evento `input`.
    */
    page: 1,
  ...
  */
  ...
  /*
    Atualiza a barra de paginação a partir do resultado de
    `paginateApprovedCandidates`. Some quando tudo cabe numa página: com 30
    candidatos, controlos de página são ruído.
  */
  function renderPaginacao({ total, page, totalPages, from, to }) {
    const barra = document.getElementById("approvedPaginacao");
    if (!barra) return;
    ...
    const proxima = document.getElementById("approvedPageNext");
    if (proxima) proxima.disabled = page >= totalPages;
    const tamanho = document.getElementById("approvedPageSize");
  ...
  function bind() {
    ...
    document
      .getElementById("approvedPagePrev")
      ?.addEventListener("click", () => irParaPagina(state.page - 1));
    document
      .getElementById("approvedPageNext")
      ?.addEventListener("click", () => irParaPagina(state.page + 1));
    document
      .getElementById("approvedPageSize")

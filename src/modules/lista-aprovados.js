...
function statusClass(status) {
  ...
  if (status === "Desistente" || status === "Documentação Rejeitada") return "danger";
...
export function createListaAprovadosController(options = {}) {
  ...
  function byId(id) {
    return state.candidates.find((row) => String(row.candidato_id) === String(id));
  }

  function listByEdital(editalId) {
    return state.lists.find((row) => String(row.edital_id) === String(editalId));
  }
  ...
  function renderKpis() {
    const summary = summarizeApprovedCandidates(state.candidates, currentFilters());
    const values = {
  ...
  function renderRows() {
    ...
    if (count) count.textContent = `${rows.length} candidato${rows.length === 1 ? "" : "s"}`;
  ...
  function renderToolbar() {
    ...
    button.title = hasActiveList ? "Incluir candidato sub judice" : "É necessário ter uma lista ativa";
  }
  ...
  async function refresh(options = {}) {
    ...
    if (error) {
      toast(`Erro ao carregar lista de aprovados: ${error.message || error}`, "error");
      return false;
    }
    ...
    state.loaded = true;
    render();
    document.dispatchEvent(new CustomEvent("agsus:listas-aprovados-loaded", { detail: { lists: state.lists } }));
    return true;
  }
  ...
  function openStatusModal(candidateId) {
    ...
    if (!candidate || !canEditCandidateStatus(profile(), candidate)) {
      return toast(candidate?.lista_ativa === false ? "A lista está inativa." : "Sem permissão para alterar o status.", "warn");
    }
    state.currentStatusCandidateId = String(candidateId);
    document.getElementById("approvedStatusCandidate").textContent = `${candidate.nome} · ${candidate.cargo}`;
    document.getElementById("approvedStatusSelect").value = candidate.status || "";
    document.getElementById("approvedStatusSei").value = candidate.processo_sei || "";
    document.getElementById("approvedStatusMatricula").value = candidate.matricula || "";
  ...
  function updateStatusRequirement() {
    ...
    if (hint) hint.textContent = required ? "Obrigatória para este status." : "Opcional para este status.";
  }
  ...
  async function saveCandidateStatus() {
    ...
    const matricula = text(document.getElementById("approvedStatusMatricula")?.value);
    ...
    loader(false);
    if (error) return toast(`Erro ao alterar status: ${error.message || error}`, "error");
  ...
  function populateSubJudiceEditais() {
    ...
    if (!select) return;
    select.innerHTML = activeLists().map((item) => `<option value="${attr(item.edital_id)}">${esc(item.edital || "Edital")} · ${esc(item.unidade || "")}</option>`).join("");
  ...
  function populateSubJudiceCargos() {
    ...
    if (!select) return;
    const cargos = uniqueCandidateCargos(state.candidates.filter((row) => String(row.edital_id) === String(editalId)));
    select.innerHTML = cargos.map((cargo) => `<option value="${attr(cargo)}">${esc(cargo)}</option>`).join("");
  }

  function openSubJudiceModal() {
    if (!canManageSubJudice(profile())) return toast("Sem permissão para incluir sub judice.", "warn");
    if (!activeLists().length) return toast("Não há lista ativa para receber sub judice.", "warn");
  ...
  async function saveSubJudice() {
    ...
    const notaRaw = text(document.getElementById("subJudiceNota")?.value).replace(",", ".");
    ...
    loader(false);
    if (error) return toast(`Erro ao incluir sub judice: ${error.message || error}`, "error");
  ...
  async function removeSubJudice(candidateId) {
    ...
    if (!window.confirm(`Remover ${candidate.nome} da lista como sub judice? O histórico será preservado.`)) return;
    loader(true, "Sub judice", "Removendo candidato...", 60);
    const { error } = await sb.rpc("remover_sub_judice", { p_candidato_id: candidate.candidato_id });
    loader(false);
    if (error) return toast(`Erro ao remover sub judice: ${error.message || error}`, "error");
  ...
  function renderImportModal() {
    ...
    document.getElementById("approvedImportEdital").textContent = state.currentImportEditalLabel || "Edital";
    ...
    const downloadCurrent = document.getElementById("approvedImportDownloadCurrent");
    ...
    if (importButton) {
      ...
      importButton.innerHTML = list ? '<i class="fa-solid fa-rotate"></i> Substituir XLSX' : '<i class="fa-solid fa-file-import"></i> Importar lista';
    }
    if (removeButton) removeButton.classList.toggle("hidden", !list || !canReplace);
    ...
    if (toggleButton) toggleButton.classList.toggle("hidden", !list || !canImport);
    const note = document.getElementById("approvedImportPermissionNote");
    if (note) {
      note.textContent = list && !canReplace
  ...
  async function openImportModal(editalId, editalLabel = "") {
    if (!canImportApprovedList(profile())) return toast("Sem permissão para gerir lista de aprovados.", "warn");
  ...
  async function importList() {
    ...
    if (current && !canReplaceApprovedList(profile())) return toast("Somente admin pode substituir uma lista existente.", "warn");
    const file = document.getElementById("approvedImportFile")?.files?.[0];
    if (!file) return toast("Selecione o arquivo XLSX.", "warn");
    if (file.size > 10 * 1024 * 1024) return toast("O XLSX deve ter no máximo 10 MB.", "warn");
    const active = document.getElementById("approvedImportActive")?.value !== "false";
    let candidates;
    try {
    ...
    loader(true, "Lista de aprovados", `Enviando ${candidates.length} candidato(s)...`, 55);
    ...
    if (upload.error) {
      loader(false);
      return toast(`Erro ao anexar XLSX: ${upload.error.message || upload.error}`, "error");
    }
    ...
    loader(false);
    if (result.error) return toast(`Erro ao importar lista: ${result.error.message || result.error}`, "error");
  ...
  async function toggleActiveList() {
    ...
    if (!list) return;
    const active = document.getElementById("approvedImportActive")?.value !== "false";
    loader(true, "Lista de aprovados", active ? "Ativando lista..." : "Inativando lista...", 60);
    const { error } = await sb.rpc("definir_lista_aprovados_ativa", { p_lista_id: list.lista_id, p_ativo: active });
    loader(false);
    if (error) return toast(`Erro ao alterar a lista: ${error.message || error}`, "error");
    toast(active ? "Lista ativada." : "Lista inativada. Os candidatos ficaram bloqueados para alteração.");
    await refresh({ loader: false });
    renderImportModal();
  }
  ...
  async function removeList() {
    ...
    if (!window.confirm("Remover a lista vigente deste edital? Os dados permanecerão preservados no histórico.")) return;
    loader(true, "Lista de aprovados", "Arquivando lista vigente...", 60);
    const { error } = await sb.rpc("remover_lista_aprovados", { p_lista_id: list.lista_id });
    loader(false);
    if (error) return toast(`Erro ao remover lista: ${error.message || error}`, "error");
  ...
  async function downloadCurrentFile() {
    ...
    const { data, error } = await sb.storage.from(BUCKET).download(list.arquivo_path);
    if (error) return toast(`Erro ao baixar XLSX: ${error.message || error}`, "error");
  ...
  function bind() {
    ...
    document.getElementById("approvedRows")?.addEventListener("click", (event) => {
      ...
      if (!button) return;
      if (button.dataset.approvedAction === "status") openStatusModal(button.dataset.candidateId);
      if (button.dataset.approvedAction === "remove-subjudice") void removeSubJudice(button.dataset.candidateId);
    });
    document.getElementById("approvedAddSubJudiceBtn")?.addEventListener("click", openSubJudiceModal);
    document.getElementById("subJudiceEdital")?.addEventListener("change", populateSubJudiceCargos);
    document.getElementById("subJudiceSave")?.addEventListener("click", () => void saveSubJudice());
    document.getElementById("approvedStatusSelect")?.addEventListener("change", updateStatusRequirement);
    document.getElementById("approvedStatusSave")?.addEventListener("click", () => void saveCandidateStatus());
    document.getElementById("approvedImportSubmit")?.addEventListener("click", () => void importList());
    document.getElementById("approvedImportToggleActive")?.addEventListener("click", () => void toggleActiveList());
    document.getElementById("approvedImportRemove")?.addEventListener("click", () => void removeList());
    document.getElementById("approvedImportDownloadCurrent")?.addEventListener("click", () => void downloadCurrentFile());
    document.querySelectorAll("[data-close-approved-modal]").forEach((button) => {
      button.addEventListener("click", () => showModal(button.dataset.closeApprovedModal, false));
    });
    ["approvedStatusModal", "subJudiceModal", "approvedImportModal"].forEach((id) => {

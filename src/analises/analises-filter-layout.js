...
function selectedCount(id){
  ...
  if(!select) return 0;
  if(select.multiple) return [...select.selectedOptions].filter(option => txt(option.value)).length;
  return txt(select.value) ? 1 : 0;
}

function activeFilterCount(){
  const selected = FILTER_IDS.reduce((total, id) => total + (selectedCount(id) > 0 ? 1 : 0), 0);
  const search = txt(document.getElementById("fBusca")?.value) ? 1 : 0;
  return selected + search;
}

function activeAdvancedCount(){
  const selected = ADVANCED_FILTER_IDS.reduce((total, id) => total + (selectedCount(id) > 0 ? 1 : 0), 0);
  const search = txt(document.getElementById("fBusca")?.value) ? 1 : 0;
  return selected + search;
}
...
function currentScopeLabel(){
  ...
  return txt(select?.selectedOptions?.[0]?.textContent || select?.value || "Ativos");
}
...
function ensureStructure(){
  ...
  if(hint) hint.textContent = "Use estes filtros somente para reduzir o recorte já carregado. A base do Supabase não é alterada.";
  ...
  if(advancedButton.parentElement !== toolbar) toolbar.appendChild(advancedButton);
  ...
  if(clear){
    ...
    clear.title = "Remove os filtros e retorna à visualização padrão de processos Ativos.";
  }
...
function updateAdvancedButton(open){
  ...
  button.title = open ? "Ocultar filtros adicionais" : "Mostrar categoria, modalidade, validação e busca";
}
...
function updateSummary(){
  ...
  const hasAnything = count > 0 || tableSearch || txt(document.getElementById("fSituacaoEdital")?.value) !== "ativo";
...
function setFiltersCollapsed(collapsed){
  ...
  button.title = collapsed ? "Mostrar os filtros da visualização" : "Ocultar os filtros da visualização";
...
function bindControls(){
  document.addEventListener("click", event => {
    ...
    if(event.target?.closest?.("#clearBtn,.multi-select-option,.multi-select-link,[data-kpi]")) scheduleUpdate();
  }, true);

  document.addEventListener("change", event => {
    if(event.target?.closest?.(".filter-panel") || event.target?.id === "fSituacaoEdital") scheduleUpdate();
  }, true);

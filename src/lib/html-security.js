const SUPABASE_CDN_PATTERN =
  /\s*<script\b[^>]*\bsrc=(['"])https:\/\/cdn\.jsdelivr\.net\/npm\/@supabase\/supabase-js@[^'"]*\1[^>]*>\s*<\/script>\s*/gi;
const ANALYTICS_FULL_URL_PATTERN =
  /page_location\s*:\s*window\.location\.href/g;
const ANALYTICS_SAFE_LOCATION =
...
export function sanitizeAnalyticsPageLocation(html) {
  return String(html ?? "").replace(
    ANALYTICS_FULL_URL_PATTERN,
    ANALYTICS_SAFE_LOCATION,
  );
}
